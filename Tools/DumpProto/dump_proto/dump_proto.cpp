...
bool BuildMobTable()
{
    ...
    while (data.Next())
    {
        ...
        fprintf(stdout, "MOB #%-5d %-16s %-16s sight: %u color %u[%s]\n", mob_table->dwVnum, mob_table->szName, mob_table->szLocaleName, mob_table->wAggressiveSight,
                mob_table->dwMobColor, 0);
    ...
    else
    {
        ...
        while (test_data.Next())	//테스트 데이터 각각을 훑어나가며,새로운 것을 추가한다.
        {
            ...
            fprintf(stdout, "[New]MOB #%-5d %-16s sight: %u color %u[%s]\n", mob_table->dwVnum, mob_table->szLocaleName, mob_table->wAggressiveSight, mob_table->dwMobColor,
