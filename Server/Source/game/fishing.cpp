...
namespace fishing
{
    ...
    */
    ...
    void Take(fishing_event_info* info, LPCHARACTER ch)
    {
        if (info->step == 1)	// 고기가 걸린 상태면..
        {
            ...
            switch (ret)
            {
                ...
                case 0:
                    ch->ChatPacket(CHAT_TYPE_INFO, "[LS;800;[IN;%d]]"/* "You have caught a fish! (%s)" */, fish_info[info->fish_id].vnum);
    ...
    void Grill(LPCHARACTER ch, LPITEM item)
    {
        ...
        ch->ChatPacket(CHAT_TYPE_INFO, "[LS;810;[IN;%d]]"/* "You are roasting %s over the fire." */, item->GetVnum());
        item->SetCount(0);
