...
static constexpr int GetAlignmentIndex(int align) noexcept
{
    if (align >= 120000)
    {
        return 0;
...
void CHARACTER::ItemDropPenalty(LPCHARACTER pkKiller)
{
    ...
    int iAlignIndex = GetAlignmentIndex(GetRealAlignment());
...
bool CHARACTER::Damage(LPCHARACTER pAttacker, int dam, EDamageType type) // returns true if dead
{
    ...
    else if (type == DAMAGE_TYPE_NORMAL || type == DAMAGE_TYPE_NORMAL_RANGE)
    {
        if (type == DAMAGE_TYPE_NORMAL)
        {
            ...
            if (GetPoint(POINT_BLOCK) && number(1, 100) <= GetPoint(POINT_BLOCK))
            {
                if (test_server)
                {
                    pAttacker->ChatPacket(CHAT_TYPE_INFO, "%s Block! (%d%%)", GetName(), GetPoint(POINT_BLOCK));
                    ChatPacket(CHAT_TYPE_INFO, "%s Block! (%d%%)", GetName(), GetPoint(POINT_BLOCK));
                }
        ...
        else if (type == DAMAGE_TYPE_NORMAL_RANGE)
        {
            ...
            if (GetPoint(POINT_DODGE) && number(1, 100) <= GetPoint(POINT_DODGE))
            {
                if (test_server)
                {
                    pAttacker->ChatPacket(CHAT_TYPE_INFO, "%s Dodge! (%d%%)", GetName(), GetPoint(POINT_DODGE));
                    ChatPacket(CHAT_TYPE_INFO, "%s Dodge! (%d%%)", GetName(), GetPoint(POINT_DODGE));
                }
    ...
    if (pAttacker)
    {
        ...
        if (pAttacker->IsPC())
        {
            ...
            /* [YMIR DEV] Damage reduced by 10% if from another empire */
    ...
    if (pAttacker)
    {
        ...
        if (m_bDetailLog)
        {
            ChatPacket(CHAT_TYPE_INFO, "[LS;915;%s;%d;%d;%d]"/* "%s[%d]'s Attack Position: %d %d" */, pAttacker->GetName(), (DWORD) pAttacker->GetVID(), pAttacker->GetX(), pAttacker->GetY());
        }
    }
