...
namespace quest
{
    ...
    /* - YOSUN_MULTILANGUAGE [MINIMAL] --------------------- */
    inline static std::string MakeLocaleKey(std::string_view tag, DWORD vnum)
    {
        std::string s;
        s.reserve(tag.size() + 12);
        s += "[";
        s += tag;
        s += ";";
        s += std::to_string(vnum);
        s += "]";
        return s;
    }
    ...
    int _item_name(lua_State* L)
    {
        if (lua_isnumber(L, 1))
        {
            ...
            /* - YOSUN_MULTILANGUAGE [MINIMAL] --------------------- */
            lua_pushstring(L, MakeLocaleKey("IN", dwVnum).c_str());
            /* ----------------------------------------------------- */
        }
    ...
    int _mob_name(lua_State* L)
    {
        if (lua_isnumber(L, 1))
        {
            DWORD dwVnum = (DWORD) lua_tonumber(L, 1);
            /* - YOSUN_MULTILANGUAGE [MINIMAL] --------------------- */
            lua_pushstring(L, MakeLocaleKey("MN", dwVnum).c_str());
            /* ----------------------------------------------------- */
        }
