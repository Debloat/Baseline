...
using TCubeValueVector = std::vector<CUBE_VALUE>;
...
using TCubeResultList          = std::vector<SCubeMaterialInfo>;
using TCubeMapByNPC            = std::unordered_map<DWORD, TCubeResultList>; // 각각의 NPC별로 어떤 걸 만들 수 있고 재료가 뭔지...
using TCubeResultInfoTextByNPC = std::unordered_map<DWORD, std::string>;	// 각각의 NPC별로 만들 수 있는 목록을 정해진 포맷으로 정리한 정보
...
bool Cube_make(LPCHARACTER ch)
{
    ...
    {
        ch->ChatPacket(CHAT_TYPE_INFO, "[LS;897]"/* "Not enough Yang or not enough space in the inventory." */);	// 이 텍스트는 이미 널리 쓰이는거라 추가번역 필요 없음
        return false;
    }
