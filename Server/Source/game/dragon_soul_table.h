...
class DragonSoulTable
{
    public:
        DragonSoulTable();
        ~DragonSoulTable();
        using TVecApplys     = std::vector <SApply>;
        using TMapApplyGroup = std::map <BYTE, TVecApplys>;
    ...
    private:
        ...
        using TMapNameToType = std::map <std::string, BYTE>;
        using TMapTypeToName = std::map <BYTE, std::string>;
