...
namespace quest
{
    ...
    class NPC
    {
        public:
            // 인자가 없는 스크립트들
            // first: state number
            using AQuestScriptType = map<int, AStateScriptType>;
            // first: quest number
            using QuestMapType = map<unsigned int, AQuestScriptType>;

            // 인자가 있는 스크립트들
            // first: state number
            using AArgQuestScriptType = map<int, vector<AArgScript >>;
            // first: quest number
            using ArgQuestMapType = map<unsigned int, AArgQuestScriptType>;
