...
class CDungeon
{
        using TPartyMap     = std::unordered_map<LPPARTY, int>;
        using TUniqueMobMap = std::map<std::string, LPCHARACTER>;
    ...
    private:
        ...
        using ItemGroupMap = std::map<std::string, ItemGroup>;
...
{
        using TDungeonMap = std::map<CDungeon::IdType, LPDUNGEON>;
        using TMapDungeon = std::map<long, LPDUNGEON>;
