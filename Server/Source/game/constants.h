...
using TMobRankStat = struct SMobRankStat
{
    int iGoldPercent;   // µ·ÀÌ ³ª¿Ã È®·ü
};

using TMobStat = struct SMobStat
{
    BYTE	byLevel;
    WORD	HP;
    DWORD	dwExp;
...
using TBattleTypeStat = struct SBattleTypeStat
{
    int		AttGradeBias;
    int		DefGradeBias;
...
using TJobInitialPoints = struct SJobInitialPoints
{
    int st;
    int ht;
    int dx;
    ...
    int max_stamina;
    int stamina_per_con;
    int stamina_per_lv_begin;
    int stamina_per_lv_end;
};

using Coord = struct __coord
{
    int		x, y;
};

using TApplyInfo = struct SApplyInfo
{
...
using TItemAttrMap = std::map<DWORD, TItemAttrTable>;
