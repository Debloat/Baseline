...
void CShopManager::Sell(LPCHARACTER ch, BYTE bCell, BYTE bCount)
{
    ...
    {
        /* [YMIR DEV] In Korea, there are many players who sell items and ask them to be restored.
        *  When selling items in stores, attribute logs are left.
        */
        {
            item->AttrLog();
        }
...
inline static bool iequals_ascii(std::string_view a, std::string_view b)
{
    if (a.size() != b.size())
        return false;
...
bool ConvertToShopItemTable(IN CGroupNode* pNode, OUT TShopTableEx& shopTable)
{
    ...
    if (iequals_ascii(stCoinType, "Gold"))
    {
        shopTable.coinType = SHOP_COIN_TYPE_GOLD;
    }

    else if (iequals_ascii(stCoinType, "SecondaryCoin"))
    {
    ...
    if (iequals_ascii(stSort, "Asc"))
    {
        std::sort(shopItems.begin(), shopItems.end(), CompareShopItemName);
    }

    else if (iequals_ascii(stSort, "Desc"))
    {
