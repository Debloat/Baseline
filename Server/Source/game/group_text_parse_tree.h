...
#include <sstream>
using TTokenVectorMap = std::map<std::string, TTokenVector>;
using TMapNameToIndex = std::map<std::string, int>;
...
class CGroupNode
{
    ...
    private:
        using TMapGroup = std::map <std::string, CGroupNode*>;
        using TMapRow = std::map <std::string, CGroupNode::CGroupNodeRow>;
