 ...
 * bCanFormat is defined by CLIENT_LOCALE_STRING
 */
    ...
    else
    {
        ...
        else
        {
            ...
            if (0 != processReturn)
            {
                if (ch->GetDesc())
                {
                    ...
                    if (pTable)
                    {
                        ...
                        {
                            /* [KaptanYosun TODO] strange call without a string */
                            len = snprintf(buf, sizeof(buf), "[LS;841]"/* "You can't use a private shop now." */, pTable->szName);
                        }

                        else
                        {
                            /* [KaptanYosun TODO] make this multilang */
                            len = snprintf(buf, sizeof(buf), "[LS;1027;%s]"/* "%s needed." */, pTable->szName);
                        }
            ...
            if (buflen > 0)
            {
                ...
                /* - CLIENT_LOCALE_STRING [EXPERIMENTAL] ---------------
                 * bCanFormat is defined by CLIENT_LOCALE_STRING
                 * hyperlinks is defined by CLIENT_LOCALE_STRING [EXPERIMENTAL]
                 */
...
struct FEmpireChatPacket
{
    ...
    FEmpireChatPacket(packet_chat & p, const char* chat_msg, int len, int iMapIndex)
        : p(p), orig_msg(chat_msg), orig_len(len), iMapIndex(iMapIndex)
    {
    }
...
int CInputMain::Chat(LPCHARACTER ch, const char* data, size_t uiBytes)
{
    ...
    if (0 != processReturn)
    {
        ...
        if (NULL != pTable)
        {
            if (3 == processReturn) //교환중
            {
                ch->ChatPacket(CHAT_TYPE_INFO, "[LS;841]"/* "You can't use a private shop now." */, pTable->szName);
            }

            else
            {
                ch->ChatPacket(CHAT_TYPE_INFO, "[LS;1027;%s]"/* "%s needed." */, pTable->szName);
            }
    ...
    switch (p->byRole)
    {
        ...
        case PARTY_ROLE_DEFENDER:
            if (ch->GetParty()->SetRole(pid, p->byRole, p->flag))
            {
                ...
                /* [KaptanYosun TODO] check what's this */
