...
ACMD(do_user_horse_feed)
{
    ...
    if (ch->CountSpecifyItem(dwFood) > 0)
    {
        ...
        ch->FeedHorse();
        ch->ChatPacket(CHAT_TYPE_INFO, "[LS;506;%s]"/* "You have fed the Horse with %s." */, ITEM_MANAGER::instance().GetTable(dwFood)->szName);
    }

    else
    {
        ch->ChatPacket(CHAT_TYPE_INFO, "[LS;507;%s]"/* "You need %s." */, ITEM_MANAGER::instance().GetTable(dwFood)->szName);
    }
}
...
{
    ch->ChatPacket(CHAT_TYPE_COMMAND, "mall https://www.mmotutkunlari.com/uye/kaptan-yosun.11389/");
    return;
}
