...
bool CClientManager::InitializeMobTable()
{
    /* [KaptanYosun Dev Note]
        Purpose:
            Load mob prototype data into m_vec_mobTable from tab-separated text files.

        Inputs:
            mob_names.txt :
                VNUM<TAB>NAME
                Optional helper file. If missing, we still load mob_proto.txt.

            mob_proto.txt :
                Main monster prototype table (tab-separated, first row = header).

        High-level flow:
            [1] Build local VNUM -> NAME lookup from mob_names.txt
            [2] Open mob_proto.txt (must exist)
            [3] Clear existing table on reload
            [4] Reserve capacity for m_vec_mobTable
            [5] Emplace and parse each mob row
            [6] Log loaded mobs
            [7] Sort by VNUM
    */

    /* - [1] Load mob_names.txt -------------------------------------------------
       We keep names in a lookup map so Set_Proto_Mob_Table can map VNUM -> display name.
       NOTE: std::string_view assumes cCsvTable returns stable pointers during this function.
    -------------------------------------------------------------------------- */
    ...
    {
        // Not fatal: mob_proto.txt can still be loaded without name overrides.
        fprintf(stderr, "Failed to load mob_names.txt\n");
    }

    else
    {
        nameData.Next(); // skip header row

        while (nameData.Next())
        {
            int vnum = 0;

            // Safer than atoi: rejects invalid/empty input.
            if (!str_to_number(vnum, nameData.AsStringByIndex(0)))
            {
                continue;
            }

            // If duplicate VNUM exists, first insert wins (by design).
            localMap.try_emplace(vnum, nameData.AsStringByIndex(1));
        }
    }

    /* - [2] Validate mob_proto.txt ---------------------------------------------
       This is the main source of truth. If missing, we cannot proceed.
    -------------------------------------------------------------------------- */
    cCsvTable data;
    ...
    /* - [3] Clear previous state (reload safe) -------------------------------- */
...
bool CClientManager::InitializeItemTable()
{
    /* [KaptanYosun Dev Note]
        Purpose:
            Load item prototype data into m_vec_itemTable and rebuild m_map_itemTableByVnum.
        ...
        Inputs:
            ...
            item_proto.txt :
                Main item prototype table (tab-separated, first row = header).

        High-level flow:
            [1] Build local VNUM -> NAME lookup from item_names.txt
            [2] Open item_proto.txt (must exist)
            [3] Clear existing tables on reload
            [4] Reserve capacity for m_vec_itemTable
            [5] Emplace and parse each item row
            [6] Rebuild m_map_itemTableByVnum from vector and sort by VNUM
    */

    /* - [1] Load item_names.txt ------------------------------------------------ */
    std::unordered_map<int, std::string_view> localMap;
    cCsvTable nameData;

    if (!nameData.Load("item_names.txt", '\t'))
    {
        // Not fatal: we can still load item_proto.txt without names.
    ...
    /* - [4] Reserve target vector capacity -------------------------------------
       We re-load the file to get an accurate row count.
       reserve() allocates raw memory only; no TItemTable objects are constructed yet.
       This prevents re-allocation during parsing.
    -------------------------------------------------------------------------- */
    data.Destroy();
    ...
    if (!data.Load("item_proto.txt", '\t'))
    {
        fprintf(stderr, "Failed to reload item_proto.txt\n");
        return false;
    }
    ...
    /* - [5] Emplace and parse rows ---------------------------------------------
       Each TItemTable is constructed directly inside the vector via emplace_back().
       Set_Proto_Item_Table fills the object in-place.
       No temporary objects, no copy/move, no index-based assignment.
    -------------------------------------------------------------------------- */
    while (data.Next())
    {
        m_vec_itemTable.emplace_back();
        TItemTable& item = m_vec_itemTable.back();

        if (!Set_Proto_Item_Table(&item, data, localMap))
        {
            fprintf(stderr, "Failed to parse item_proto row.\n");
            m_vec_itemTable.pop_back();
            continue;
        }
    }
    ...
    /* - [6] Rebuild map + sort -------------------------------------------------
       Important:
            m_map_itemTableByVnum stores pointers into m_vec_itemTable.
            Therefore:
                - Build the map AFTER vector is fully sized/filled.
                - Do NOT store these pointers if vector can reallocate later.
    -------------------------------------------------------------------------- */
    m_map_itemTableByVnum.clear();
    ...
    for (auto& item : m_vec_itemTable)
    {
        sys_log(1, "ITEM: #%-5lu %-24s VAL: %ld %ld %ld %ld %ld %ld WEAR %lu ANTI %lu IMMUNE %lu REFINE %lu REFINE_SET %u MAGIC_PCT %u",
                ...
                item.dwRefinedVnum,
                item.wRefineSet,
                item.bAlterToMagicItemPct);

        m_map_itemTableByVnum.emplace(item.dwVnum, &item);
    }
