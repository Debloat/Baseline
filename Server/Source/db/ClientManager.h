...
{
    public:
        using TPeerList              = std::list<CPeer*>;
        using TPlayerTableCacheMap   = std::unordered_map<DWORD, CPlayerTableCache*>;
        using TItemCacheMap          = std::unordered_map<DWORD, CItemCache*>;
        using TItemCacheSet          = std::unordered_set<CItemCache*>;
        using TItemCacheSetPtrMap    = std::unordered_map<DWORD, TItemCacheSet*>;
        using TItemPriceListCacheMap = std::unordered_map<DWORD, CItemPriceListTableCache*>;
        using TChannelStatusMap      = std::unordered_map<short, BYTE>;
         ...
         * second: 요청한 플레이어의 ID
         */
        using TItemPricelistReqInfo = std::pair<DWORD, DWORD>;
    ...
    private:
        ...
        using TLoginDataByLoginKey = std::unordered_map<DWORD, CLoginData*>;
        TLoginDataByLoginKey			m_map_pkLoginData;

        // Login LoginData pair
        using TLoginDataByLogin = std::unordered_map<std::string, CLoginData*>;
        TLoginDataByLogin			m_map_pkLoginDataByLogin;

        // AccountID LoginData pair
        using TLoginDataByAID = std::unordered_map<DWORD, CLoginData*>;
        TLoginDataByAID				m_map_pkLoginDataByAID;

        // Login LoginData pair (실제 로그인 되어있는 계정)
        using TLogonAccountMap = std::unordered_map<std::string, CLoginData*>;
        ...
        using TPartyMember     = std::map<DWORD, TPartyInfo>;
        using TPartyMap        = std::map<DWORD, TPartyMember>;
        using TPartyChannelMap = std::map<BYTE, TPartyMap>;
        TPartyChannelMap m_map_pkChannelParty;

        using TEventFlagMap    = std::map<std::string, long>;
    ...
    private:
        ...
        using TLogoutPlayerMap = std::unordered_map<DWORD, TLogoutPlayer*>;
