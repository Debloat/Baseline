...
bool Set_Proto_Mob_Table(TMobTable *mobTable, cCsvTable &csvTable, const std::unordered_map<int, std::string_view>& nameMap)
{
    ...
    if (auto it = nameMap.find(mobTable->dwVnum); it != nameMap.end())
    {
        ...
        strlcpy(mobTable->szName, name.data(), sizeof(mobTable->szName));
    }
...
bool Set_Proto_Item_Table(TItemTable *itemTable, cCsvTable &csvTable, const std::unordered_map<int, std::string_view>& nameMap)
{
    int col = 0;
