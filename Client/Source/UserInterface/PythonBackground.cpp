...
void CPythonBackground::SetViewDistanceSet(int eNum, float fFarClip)
{
    ...
    m_ViewDistanceSet[eNum].m_fMinViewDistance = fFarClip * 0.5f;//0.3333333f;
    m_ViewDistanceSet[eNum].m_fMaxViewDistance = fFarClip * 0.7f;//0.6666667f;
...
void CPythonBackground::GetDistanceSetInfo(int* peNum, float* pfStart, float* pfEnd, float* pfFarClip)
{
    ...
    *pfStart = m_ViewDistanceSet[m_eViewDistanceNum].m_fMinViewDistance;
    *pfEnd = m_ViewDistanceSet[m_eViewDistanceNum].m_fMaxViewDistance;
...
void CPythonBackground::Render()
{
...

    {
        ...
        const D3DXVECTOR3& eye = pCamera->GetEye();
        ...
        // --- SHADOW (SCALARS) ---
        frame.shadowBias = es.runtime.shadowBias;
        frame.shadowDarkness = es.runtime.shadowDarkness;
        frame.invShadowMapSize = 1.0f / es.runtime.shadowMapSize;

        // push to provider
        if (IShaderProvider* sp = GetShaderProviderMutable())
        {
            sp->SetFrameShaderInputs(frame);
        }
    }
