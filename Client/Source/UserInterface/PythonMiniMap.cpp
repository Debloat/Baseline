...
void CPythonMiniMap::Render(float fScreenX, float fScreenY)
{
    ...
    if (!m_MiniMapFilterGraphicImageInstance.IsEmpty() &&
        m_MiniMapFilterGraphicImageInstance.GetTexturePointer())
    {
        pMaskTex = m_MiniMapFilterGraphicImageInstance.GetTexturePointer()->GetD3DTexture();
    }
    STATEMANAGER.SetTexture(1, pMaskTex);
    ...
    if (spIcons && spIcons->BindPipelineState(MiniMapIconPipeline))
    {
        ...
        if (m_fScale >= 2.0f)
        {
            ...
            UploadColor(CInstanceBase::GetIndexedNameColor(CInstanceBase::NAMECOLOR_MOB));
            ...
            DWORD lastColor = 0xFFFFFFFF;
            while (aIterator != m_OtherPCPositionVector.end())
            {
                TMarkPosition& rPosition = *aIterator;
                if (DWORD col = CInstanceBase::GetIndexedNameColor(rPosition.m_eNameColor); col != lastColor)
                {
                    UploadColor(col);
                    lastColor = col;
                }
        ...
        UploadColor(CInstanceBase::GetIndexedNameColor(CInstanceBase::NAMECOLOR_NPC));
        ...
        UploadColor(CInstanceBase::GetIndexedNameColor(CInstanceBase::NAMECOLOR_WARP));
