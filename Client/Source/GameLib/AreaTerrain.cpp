...
LPDIRECT3DTEXTURE9 CTerrain::AddTexture32(BYTE byImageNum, BYTE * pbyImage, long lTextureWidth, long lTextureHeight)
{
    ...
    hr = ms_lpd3dDevice->CreateTexture(
             ...
             D3DFMT_A8R8G8B8, D3DPOOL_DEFAULT, &pkTex, nullptr);
...
void CTerrain::_CalculateTerrainPatch(BYTE byPatchNumX, BYTE byPatchNumY)
{
    ...
    /* - Water Mesh Resolution Increase -------------------- */
    // Water mesh density per terrain cell (2 = 2x2, 4 = 4x4). Start with 2.
    static constexpr int WATER_SUBDIV = 8;

    const UINT uMaxWaterVerts = (CTerrainImpl::PATCH_XSIZE * CTerrainImpl::PATCH_YSIZE) * (WATER_SUBDIV * WATER_SUBDIV) * 6;

    std::vector<SWaterVertex> waterVerts;
    waterVerts.reserve(uMaxWaterVerts);
    /* ----------------------------------------------------- */
                ...
                if (byNumWater != 0xFF)
                {
                    ...
                    if (-1 != lWaterHeight)
                    {
                        ...
                        if (dwAlphaKey != 0)
                        {
                            // Corner alphas:
                            // dwAlpha0 = TL (fX,            -fY)
                            // dwAlpha1 = BL (fX,            -fY - CELLSCALE)
                            // dwAlpha2 = TR (fX + CELLSCALE,-fY)
                            // dwAlpha3 = BR (fX + CELLSCALE,-fY - CELLSCALE)
                            const float aTL = (float)dwAlpha0;
                            const float aBL = (float)dwAlpha1;
                            const float aTR = (float)dwAlpha2;
                            const float aBR = (float)dwAlpha3;

                            const float baseX = fX;
                            const float baseY = -fY;
                            const float z = (float)lWaterHeight * m_fHeightScale;

                            const float step = float(CELLSCALE) / float(WATER_SUBDIV);

                            auto AlphaBilinear = [&](float u, float v) -> DWORD
                                {
                                    // u,v in [0..1], v increases downward
                                    const float aTop = aTL + (aTR - aTL) * u;
                                    const float aBot = aBL + (aBR - aBL) * u;
                                    float a = aTop + (aBot - aTop) * v;
                            ...
                            for (int sy = 0; sy < WATER_SUBDIV; ++sy)
                            {
                                for (int sx = 0; sx < WATER_SUBDIV; ++sx)
                                {
                                    // Sub-quad corners in world space
                                    const float x0 = baseX + step * float(sx);
                                    const float x1 = baseX + step * float(sx + 1);

                                    // Y goes downward as negative in your mesh
                                    const float y0 = baseY - step * float(sy);
                                    const float y1 = baseY - step * float(sy + 1);

                                    // Sub-quad UVs in [0..1] across the original cell
                                    const float localu0 = float(sx) / float(WATER_SUBDIV);
                                    const float localu1 = float(sx + 1) / float(WATER_SUBDIV);
                                    const float localv0 = float(sy) / float(WATER_SUBDIV);
                                    const float localv1 = float(sy + 1) / float(WATER_SUBDIV);

                                    constexpr float WATER_UV_SCALE = 1.0f / 512.0f;

                                    const float u0 = x0 * WATER_UV_SCALE;
                                    const float u1 = x1 * WATER_UV_SCALE;
                                    const float v0 = y0 * WATER_UV_SCALE;
                                    const float v1 = y1 * WATER_UV_SCALE;

                                    const DWORD A00 = AlphaBilinear(localu0, localv0);
                                    const DWORD A01 = AlphaBilinear(localu0, localv1);
                                    const DWORD A10 = AlphaBilinear(localu1, localv0);
                                    const DWORD A11 = AlphaBilinear(localu1, localv1);

                                    // Ensure we don't overflow the allocated buffer
                                    assert(waterVerts.size() + 6 <= uMaxWaterVerts);

                                    // Triangle 1: (x0,y0,u0,v0) (x0,y1,u0,v1) (x1,y0,u1,v0)
                                    waterVerts.emplace_back(x0, y0, z, u0, v0, (A00 << 24) | 0x00FFFFFF);
                                    waterVerts.emplace_back(x0, y1, z, u0, v1, (A01 << 24) | 0x00FFFFFF);
                                    waterVerts.emplace_back(x1, y0, z, u1, v0, (A10 << 24) | 0x00FFFFFF);
                                    // Triangle 2: (x1,y0,u1,v0) (x0,y1,u0,v1) (x1,y1,u1,v1)
                                    waterVerts.emplace_back(x1, y0, z, u1, v0, (A10 << 24) | 0x00FFFFFF);
                                    waterVerts.emplace_back(x0, y1, z, u0, v1, (A01 << 24) | 0x00FFFFFF);
                                    waterVerts.emplace_back(x1, y1, z, u1, v1, (A11 << 24) | 0x00FFFFFF);
    ...
    {
        /* - Water Mesh Resolution Increase -------------------- */
        rkTerrainPatch.BuildWaterVertexBuffer(waterVerts.data(), waterVerts.size());
        /* ----------------------------------------------------- */
    }
