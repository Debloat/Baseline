...
{
    // --- Scene environment time reset (new map lifecycle) ---
    {
        auto& es = GetEnvironmentShaderSettings();
        es.runtime.timeSeconds = 0.0f;
        es.runtime.windDirection = es.authoring.windDirection;
        es.runtime.windStrength = es.authoring.windStrength;
    }
    ...
    m_lpSceneDepthTexture = nullptr;
    m_lpSceneDepthRenderTargetSurface = nullptr;
    m_lpSceneDepthDepthSurface = nullptr;

    m_lpSceneDepthBackupRT = nullptr;
    m_lpSceneDepthBackupDepth = nullptr;
...
void CMapOutdoor::SetEnvironmentSkyBox()
{
    ...
    m_SkyBox.SetTexture(mc_pEnvironmentData->strSkyBoxTextureFileName.c_str());
