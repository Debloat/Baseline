...
bool CMapOutdoor::BeginRenderCharacterShadowToTexture()
{
    ...
    D3DXVECTOR3 v3Eye = v3Target - sunDir * 1250.0f;
    /* ----------------------------------------------------- */
...
void CMapOutdoor::CreateSceneDepthTexture()
{
    ReleaseSceneDepthTexture();

    D3DVIEWPORT9 vp;
    ms_lpd3dDevice->GetViewport(&vp);

    UINT width = vp.Width;
    UINT height = vp.Height;

    m_SceneDepthViewport.X = 0;
    m_SceneDepthViewport.Y = 0;
    m_SceneDepthViewport.Width = width;
    m_SceneDepthViewport.Height = height;
    m_SceneDepthViewport.MinZ = 0.0f;
    m_SceneDepthViewport.MaxZ = 1.0f;
    ...
    if (FAILED(ms_lpd3dDevice->CreateTexture(
        ...
        D3DFMT_A8R8G8B8,
        D3DPOOL_DEFAULT,
        &m_lpSceneDepthTexture,
        nullptr)))
    {
        return;
    }

    m_lpSceneDepthTexture->GetSurfaceLevel(
        0,
        &m_lpSceneDepthRenderTargetSurface);

    ms_lpd3dDevice->CreateDepthStencilSurface(
        width,
        height,
        D3DFMT_D16,
        D3DMULTISAMPLE_NONE,
        0,
        TRUE,
        &m_lpSceneDepthDepthSurface,
        nullptr);
}

void CMapOutdoor::ReleaseSceneDepthTexture()
{
    SAFE_RELEASE(m_lpSceneDepthRenderTargetSurface);
    SAFE_RELEASE(m_lpSceneDepthDepthSurface);
    SAFE_RELEASE(m_lpSceneDepthTexture);
}

bool CMapOutdoor::BeginRenderSceneDepth()
{
    if (!m_lpSceneDepthTexture)
        CreateSceneDepthTexture();

    if (!m_lpSceneDepthTexture)
        return false;

    if (FAILED(ms_lpd3dDevice->GetRenderTarget(0, &m_lpSceneDepthBackupRT)))
        return false;

    if (FAILED(ms_lpd3dDevice->GetDepthStencilSurface(&m_lpSceneDepthBackupDepth)))
        return false;

    ms_lpd3dDevice->GetViewport(&m_SceneDepthBackupViewport);

    ms_lpd3dDevice->SetRenderTarget(0, m_lpSceneDepthRenderTargetSurface);
    ms_lpd3dDevice->SetDepthStencilSurface(m_lpSceneDepthDepthSurface);

    ms_lpd3dDevice->SetViewport(&m_SceneDepthViewport);
...
void CMapOutdoor::EndRenderSceneDepth()
{
    ms_lpd3dDevice->SetViewport(&m_SceneDepthBackupViewport);

    ms_lpd3dDevice->SetRenderTarget(0, m_lpSceneDepthBackupRT);
    ms_lpd3dDevice->SetDepthStencilSurface(m_lpSceneDepthBackupDepth);

    SAFE_RELEASE(m_lpSceneDepthBackupRT);
    SAFE_RELEASE(m_lpSceneDepthBackupDepth);
}
