...
struct ImGuiTextBuffer
{
    ...
    {
        if (Buf.Size > size) Buf.Data[size] = 0;    // Similar to resize(0) on ImVector: empty string but don't free buffer.
...
struct ImFontAtlas
{
    ...
    bool            IsBuilt() const
    {
        return Fonts.Size > 0 && TexIsBuilt;    // Bit ambiguous: used to detect when user didn't build texture but effectively we should check TexID != 0 except that would be backend dependent..
    }
