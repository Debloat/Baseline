    ...
    while (begin < end)
    {
        ...
        else
        {
            if (defCodePage == CP_ARABIC) // ARABIC
            {
                ...
                /* - CLIENT_LOCALE_STRING [EXPERIMENTAL] --------------- */
...
void CGraphicTextInstance::__BuildGeometry(RECT* pClipRect)
{
    CGraphicText* pkText = m_roText.GetPointer();
    if (!pkText)
        return;

    if (CGraphicFontTexture* pFontTexture = pkText->GetFontTexturePointer(); !pFontTexture)
        return;

    // Cache clip dependency (your build currently skips when fCurY <= pClipRect->top)
    if (pClipRect)
    {
        m_hasCachedClip = true;
        m_cachedClipTop = pClipRect->top;
    }
    else
    {
        m_hasCachedClip = false;
        m_cachedClipTop = 0;
    }
    ...
    // Reserve to reduce realloc
    m_batchVertices.reserve(m_pCharInfoVector.size() * 4);
    m_batchIndices.reserve(m_pCharInfoVector.size() * 6);
    m_batchRuns.reserve(4);

    int currentPageIndex = -1;
    UINT runIndexStart = 0;

    auto AppendQuad = [&](float sx, float sy, float ex, float ey,
        float u0, float v0, float u1, float v1,
        DWORD color)
        {
            const WORD base = static_cast<WORD>(m_batchVertices.size());

            TPDTVertex v{};
            v.position.z = m_v3Position.z;

            // v0 (top-left)
            v.position.x = sx; v.position.y = sy;
            v.diffuse = color;
            v.texCoord.x = u0; v.texCoord.y = v0;
            m_batchVertices.push_back(v);

            // v1 (bottom-left)
            v.position.x = sx; v.position.y = ey;
            v.texCoord.x = u0; v.texCoord.y = v1;
            m_batchVertices.push_back(v);

            // v2 (top-right)
            v.position.x = ex; v.position.y = sy;
            v.texCoord.x = u1; v.texCoord.y = v0;
            m_batchVertices.push_back(v);

            // v3 (bottom-right)
            v.position.x = ex; v.position.y = ey;
            v.texCoord.x = u1; v.texCoord.y = v1;
            m_batchVertices.push_back(v);

            // Your current (working) winding:
            m_batchIndices.push_back(base + 0);
            m_batchIndices.push_back(base + 1);
            m_batchIndices.push_back(base + 2);
            m_batchIndices.push_back(base + 2);
            m_batchIndices.push_back(base + 1);
            m_batchIndices.push_back(base + 3);
        };

    auto FlushRunIfAny = [&](int pageIndexToFlush)
        {
            const UINT now = m_batchIndices.size();
            if (currentPageIndex == -1)
                return;
            if (now <= runIndexStart)
                return;

            STextBatchRun run{};
            run.pageIndex = pageIndexToFlush;
            run.indexStart = runIndexStart;
            run.indexCount = now - runIndexStart;
            m_batchRuns.push_back(run);
        };

    auto BeginNewRun = [&](int newPageIndex)
        {
            currentPageIndex = newPageIndex;
            runIndexStart = m_batchIndices.size();
        };

    // ------------------------------------------------------------
    // Recompute fStanX / fStanY (copied from Render)
    // ------------------------------------------------------------
    ...
    // ------------------------------------------------------------
    // Outline pass (cached into runs)
    // ------------------------------------------------------------
    if (auto const& tm = GetYosunControlSettings().textMetrics; m_isOutline && !tm.forceDisableOutline)
    {
        fCurX = fStanX;
        fCurY = fStanY;
        fFontMaxHeight = 0.0f;

        for (auto it = m_pCharInfoVector.begin(); it != m_pCharInfoVector.end(); ++it)
        {
            pCurCharInfo = *it;
            ...
            if (const int pageIndex = static_cast<int>(pCurCharInfo->index); currentPageIndex != pageIndex)
            {
                if (currentPageIndex != -1)
                    FlushRunIfAny(currentPageIndex);

                BeginNewRun(pageIndex);
            }

            const float u0 = pCurCharInfo->left;
            const float v0 = pCurCharInfo->top;
            const float u1 = pCurCharInfo->right;
            const float v1 = pCurCharInfo->bottom;

            const DWORD color = m_dwOutLineColor;
            float feather = 0.0f;

            const float sx = (fCurX - 0.5f) - feather;
            const float sy = (fCurY - 0.5f) - feather;
            const float ex = sx + fFontWidth + feather * 2.0f;
            const float ey = sy + fFontHeight + feather * 2.0f;

            AppendQuad(sx - fFontHalfWeight, sy, ex - fFontHalfWeight, ey, u0, v0, u1, v1, color);
            AppendQuad(sx + fFontHalfWeight, sy, ex + fFontHalfWeight, ey, u0, v0, u1, v1, color);
            AppendQuad(sx, sy - fFontHalfWeight, ex, ey - fFontHalfWeight, u0, v0, u1, v1, color);
            AppendQuad(sx, sy + fFontHalfWeight, ex, ey + fFontHalfWeight, u0, v0, u1, v1, color);
    ...
    for (int i = 0; i < (int)m_pCharInfoVector.size(); ++i)
    {
        ...
        AppendQuad(
            fFontSx, fFontSy, fFontEx, fFontEy,
...
void CGraphicTextInstance::Render(RECT * pClipRect)
{
    ...
    ++tm.totalTextInstances;
    tm.totalChars += m_pCharInfoVector.size();
    ...
    UINT bbW = 0, bbH = 0;
    CGraphicBase::GetBackBufferSize(&bbW, &bbH);

    TextShaderInputs tin{};
    tin.invScreenW = (bbW > 0) ? (1.0f / float(bbW)) : 0.0f;
    tin.invScreenH = (bbH > 0) ? (1.0f / float(bbH)) : 0.0f;

    CGraphicDevice::UploadTextConstants(tin);

    {
        // ------------------------------------------------------------
        // Stage 2: cached geometry (rebuild only when needed)
        // ------------------------------------------------------------
        bool clipChanged = false;
        if (pClipRect)
        {
            if (!m_hasCachedClip || m_cachedClipTop != pClipRect->top)
                clipChanged = true;
        }
        else
        {
        ...
        if (m_isGeometryDirty || clipChanged)
        {
            __BuildGeometry(pClipRect);
        }

        // Draw cached runs
        for (const auto& run : m_batchRuns)
        {
            pFontTexture->SelectTexture(run.pageIndex);
            STATEMANAGER.SetTexture(0, pFontTexture->GetD3DTexture());

            const UINT vtxCount = m_batchVertices.size();
            const UINT primCount = run.indexCount / 3;
            ...
            STATEMANAGER.DrawIndexedPrimitiveUP(
                ...
                vtxCount,
                primCount,
                m_batchIndices.data() + run.indexStart,
                D3DFMT_INDEX16,
    ...
    tm.peakChars         = (tm.totalChars         > tm.peakChars)         ? tm.totalChars         : tm.peakChars;
    tm.peakDrawCalls     = (tm.totalDrawCalls     > tm.peakDrawCalls)     ? tm.totalDrawCalls     : tm.peakDrawCalls;
    tm.peakTextInstances = (tm.totalTextInstances > tm.peakTextInstances) ? tm.totalTextInstances : tm.peakTextInstances;
}
