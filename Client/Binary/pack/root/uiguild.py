...
class GuildWindow(ui.ScriptWindow):
	...
	def __LoadWindow(self):
		...
		try:
			...
			if localeInfo.IsARABIC():
				skillPagePath = uiScriptLocale.LOCALE_UISCRIPT_PATH + "guildwindow_guildskillpage.py"
			else:
				skillPagePath = "uiscript/guildwindow_guildskillpage.py"
			...
			self.pageWindow = {
				...
				"SKILL":		self.PageWindow(self, skillPagePath),
...
class BuildGuildBuildingWindow(ui.ScriptWindow):
	...
	def __SetBuildingData(self, data):
		self.buildingPriceValue.SetText(localeInfo.NumberToMoneyString(data["PRICE"]))
